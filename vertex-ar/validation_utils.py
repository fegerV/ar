...
class EnhancedValidator:
    ...
    def validate_phone_number(cls, phone: str, region: str = 'international') -> str:
        ...
        if not re.match(pattern, normalized_phone):
            ValidationLogger.log_validation_error(
                "phone", phone, f"Invalid phone format for region: {region}"
            )
            raise ValidationError(
                f"Invalid phone number format for region: {region}",
                field="phone",
                value=phone,
    ...
    def validate_file_size(cls, file_content: bytes, max_size: int, file_type: str = "file") -> None:
        ...
        ValidationLogger.log_validation_success(
            f"{file_type}_size", file_size, {"max_size": max_size}
        )
    ...
    def validate_image_content(cls, file_content: bytes, filename: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if mime_type not in allowed_types:
                ValidationLogger.log_validation_error(
                    "image_mime", mime_type, f"Unsupported image type: {mime_type}"
                )
                raise ValidationError(
        ...
        ValidationLogger.log_validation_success(
            "image_content", metadata, {"filename": filename}
        )
        return metadata
    ...
    def validate_video_content(cls, file_content: bytes, filename: str = None) -> Dict[str, Any]:
        ...
        if file_content[4:8] != b'ftyp':
            ...
            raise ValidationError(
                "Invalid video format. Expected MP4 file with ftyp header",
                field="video_format",
                value="unknown"
            )
        ...
        if brand not in valid_brands:
            ValidationLogger.log_validation_error(
                "video_brand", brand.decode('utf-8', errors='ignore'), 
                f"Unsupported MP4 brand: {brand}"
            )
            raise ValidationError(
        ...
        try:
            ...
            if mime_type != 'video/mp4':
                ValidationLogger.log_validation_error(
                    "video_mime", mime_type, f"Invalid video MIME type: {mime_type}"
                )
                raise ValidationError(
                    f"Invalid video type: {mime_type}. Expected: video/mp4",
                    field="video_type",
                    value=mime_type
                )
        except Exception:
        ...
        metadata = {
            "mime_type": mime_type,
            "brand": brand.decode('utf-8', errors='ignore'),
            "size_bytes": len(file_content)
        }
        
        ValidationLogger.log_validation_success(
            "video_content", metadata, {"filename": filename}
        )
        return metadata
