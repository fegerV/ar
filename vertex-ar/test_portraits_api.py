...
def test_create_order():
    ...
    files = {"image": open("/tmp/test_portrait.jpg", "rb"), "video": open("/tmp/test_video.mp4", "rb")}
    data = {"phone": "+79991234567", "name": "Тестовый Клиент"}

    response = requests.post(f"{BASE_URL}/orders/create", headers={"Authorization": f"Bearer {TOKEN}"}, files=files, data=data)
...
def test_search_clients(phone):
    ...
    response = requests.get(
        f"{BASE_URL}/clients/search", headers={"Authorization": f"Bearer {TOKEN}"}, params={"phone": phone}
    )
...
def test_list_clients():
    ...
    response = requests.get(f"{BASE_URL}/clients/list", headers={"Authorization": f"Bearer {TOKEN}"})
...
def test_list_portraits(client_id=None):
    ...
    response = requests.get(f"{BASE_URL}/portraits/list", headers={"Authorization": f"Bearer {TOKEN}"}, params=params)
...
def test_add_video(portrait_id):
    ...
    files = {"video": open("/tmp/test_video.mp4", "rb")}
    data = {"portrait_id": portrait_id}

    response = requests.post(f"{BASE_URL}/videos/add", headers={"Authorization": f"Bearer {TOKEN}"}, files=files, data=data)
...
def test_list_videos(portrait_id):
    ...
    response = requests.get(f"{BASE_URL}/videos/list/{portrait_id}", headers={"Authorization": f"Bearer {TOKEN}"})
...
def test_activate_video(video_id):
    ...
    response = requests.put(f"{BASE_URL}/videos/{video_id}/activate", headers={"Authorization": f"Bearer {TOKEN}"})
...
def test_portrait_details(portrait_id):
    ...
    response = requests.get(f"{BASE_URL}/portraits/{portrait_id}/details", headers={"Authorization": f"Bearer {TOKEN}"})
