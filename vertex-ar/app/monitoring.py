...
class SystemMonitor:
    ...
    def get_memory_usage(self) -> Dict[str, Any]:
        ...
        try:
            for proc in psutil.process_iter(['pid', 'name', 'memory_info', 'memory_percent']):
                try:
                    proc_info = proc.info
                    if proc_info['memory_percent'] and proc_info['memory_percent'] > 1:  # Only include processes using >1% memory
        ...
        return {
            "virtual": {
                ...
                "details": {k: v / (1024**3) if isinstance(v, int) and v > 1024**3 else v 
    ...
    def _get_disk_temperature(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if os.path.exists('/usr/sbin/hddtemp'):
                try:
                    result = subprocess.run(['/usr/sbin/hddtemp', '/dev/sd*', '/dev/nvme*'], 
            ...
            # Try smartctl as fallback
                ...
                try:
                    result = subprocess.run(['lsblk', '-d', '-o', 'NAME'], 
    ...
    def _get_recent_errors(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "log_files": log_files,
                "total_errors": sum(len(f["recent_errors"]) for f in log_files)
            }
    ...
    def get_historical_trends(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            "period_hours": hours,
            "data_points": len(recent_data) if recent_data else 0,
            "trends": trends
        }
        ...
        try:
            ...
            if cpu_usage["percent"] > self.alert_thresholds["cpu"]:
                ...
                health_data["alerts"].append({
                    "type": "cpu",
                    "severity": "high" if cpu_usage["percent"] > 90 else "medium",
                    "message": alert_msg
                })
            ...
            if disk_info["storage"]["percent"] > self.alert_thresholds["disk"]:
                alert_msg = f"High disk usage: {disk_info['storage']['percent']:.1f}% ({disk_info['storage']['used_gb']:.1f}GB used)"
                health_data["alerts"].append({
                    "type": "disk",
                ...
                if service_name in ["database", "storage", "minio", "web_server"]:
                    if not service_data["healthy"]:
                        ...
                        await alert_manager.send_alert(
                            ...
                            f"Service {service_name} Failure",
                            alert_msg,
                            "high"
                        )
                    elif service_data.get("response_time_ms", 0) > 5000:  # 5 seconds
            ...
            if "external_services" in service_health:
                for ext_service, ext_data in service_health["external_services"].items():
                    if not ext_data["healthy"]:
                        ...
                        await alert_manager.send_alert(
                            f"external_{ext_service}",
                            f"External Service {ext_service.title()} Failure",
                            alert_msg,
                            "medium"
                        )
