...
class FolderService:
    ...
    def is_default_company(self, company: Dict[str, Any]) -> bool:
        """
        Check if the company is the default Vertex AR company.

        Args:
            company: Company dictionary from database

        Returns:
            True if this is the default company, False otherwise
        """
        return company.get('id') == 'vertex-ar-default'

    def get_effective_company_slug(self, company: Dict[str, Any]) -> str:
        """
        Get the effective company slug for path construction.
        For the default company, returns empty string to avoid redundant path segments.

        Args:
            company: Company dictionary from database

        Returns:
            Company slug or empty string for default company
        """
        if self.is_default_company(company):
            return ""  # No slug for default company to avoid redundancy
        return self.get_company_slug(company)
    ...
    def build_order_path(
        ...
        Note: For default company, company_slug is omitted to avoid redundancy.
        ...
        # Get effective company slug (empty for default company)
        company_slug = self.get_effective_company_slug(company)
        ...
        if company_slug:
            ...
            # For default company, skip the company_slug segment
            path = self.storage_root / folder_path / content_type / order_id
    ...
    def build_relative_path(
        ...
        parts.extend([content_type, order_id])
        if subfolder:
    ...
    def provision_company_hierarchy(
        ...
        logger.info(
            ...
            company_slug=company_slug if company_slug else "(default - no slug)",
        ...
        try:
            for category_slug in category_slugs:
                ...
                # Build base category path (without company slug for default company)
                if company_slug:
                    base_path = self.storage_root / folder_path / company_slug / category_slug
                else:
                    # For default company, skip the company_slug segment
                    base_path = self.storage_root / folder_path / category_slug
    ...
    def verify_hierarchy(
        ...
        for category_slug in category_slugs:
            # Build base path (without company slug for default company)
            if company_slug:
                base_path = self.storage_root / folder_path / company_slug / category_slug
            else:
                # For default company, skip the company_slug segment
