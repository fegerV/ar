...
from app.middleware import RequestLoggingMiddleware, ErrorLoggingMiddleware, ValidationErrorLoggingMiddleware, AdminCSPMiddleware
...
def create_app() -> FastAPI:
    ...
    # Add CSP middleware for Content Security Policy (admin panel only)
            ...
            async def cleanup_expired_sessions():
                ...
                while True:
                    try:
                        ...
                        if hasattr(tokens, 'cleanup_expired_sessions'):
                            cleaned_count = tokens.cleanup_expired_sessions()
                            if cleaned_count > 0:
                                logger.info(f"Cleaned up {cleaned_count} expired sessions")
                    except Exception as e:
                        logger.error("Error cleaning up expired sessions", error=str(e), exc_info=e)

            asyncio.create_task(cleanup_expired_sessions())
            logger.info("Session cleanup task started")

        except Exception as e:
            logger.error("Failed to start session cleanup task", error=str(e), exc_info=e)
    ...
    # Store app instance for access in modules
    _app_instance = app
