...
async def create_video(
    portrait_id: str, video: UploadFile = File(...), is_active: bool = False, username: str = Depends(get_current_user)
) -> VideoResponse:
...
async def list_videos(portrait_id: str, username: str = Depends(get_current_user)) -> List[VideoResponse]:
...
async def get_active_video(portrait_id: str, username: str = Depends(get_current_user)) -> VideoResponse:
...
async def set_active_video(video_id: str, username: str = Depends(get_current_user)) -> VideoResponse:
    ...
    if not updated:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to set active video")
...
async def get_video(video_id: str, username: str = Depends(get_current_user)) -> VideoResponse:
    """Get video by ID."""
    database = get_database()
    video = database.get_video(video_id)
...
async def delete_video(video_id: str, username: str = Depends(get_current_user)):
    """Delete video."""
    database = get_database()
