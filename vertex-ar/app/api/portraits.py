...
async def create_portrait(
    client_id: str, image: UploadFile = File(...), username: str = Depends(get_current_user)
) -> PortraitResponse:
...
async def list_portraits(client_id: str = None, username: str = Depends(get_current_user)) -> List[PortraitResponse]:
...
async def get_portrait(portrait_id: str, username: str = Depends(get_current_user)) -> PortraitResponse:
    """Get portrait by ID."""
    database = get_database()
...
async def delete_portrait(portrait_id: str, username: str = Depends(get_current_user)):
    """Delete portrait."""
    database = get_database()
    ...
    if not deleted:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to delete portrait")
