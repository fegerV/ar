...
async def change_password(
    ...
    if not _verify_password(password_change.current_password, user["password_hash"]):
        logger.warning("password_change_failed_invalid_current_password", username=current_user)
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Current password is incorrect")
    
    # Hash new password
    new_password_hash = _hash_password(password_change.new_password)
    
    # Update password
    success = database.update_user(current_user, password_hash=new_password_hash)
    if not success:
    ...
    tokens = get_token_manager()
    tokens.revoke_user_except_current(current_user)
