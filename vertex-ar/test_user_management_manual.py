...
def test_user_management():
    ...
    response = requests.post(f"{BASE_URL}/auth/login", json={"username": "admin", "password": "admin123456"})
    ...
    response = requests.get(f"{BASE_URL}/users/profile", headers={"Authorization": f"Bearer {admin_token}"})
    ...
    response = requests.post(f"{BASE_URL}/auth/login", json={"username": "testuser", "password": "test123456"})
    ...
    response = requests.put(
        ...
        json={"email": "updated@example.com", "full_name": "Updated User Name"},
    ...
    response = requests.get(f"{BASE_URL}/users/users", headers={"Authorization": f"Bearer {admin_token}"})
    ...
    response = requests.get(f"{BASE_URL}/users/stats", headers={"Authorization": f"Bearer {admin_token}"})
    ...
    response = requests.get(f"{BASE_URL}/users/users/search?query=test", headers={"Authorization": f"Bearer {admin_token}"})
    ...
    response = requests.get(f"{BASE_URL}/users/users", headers={"Authorization": f"Bearer {user_token}"})
    ...
    if response.status_code == 200:
        ...
        response = requests.post(f"{BASE_URL}/auth/login", json={"username": "testuser", "password": "newpassword123"})
    ...
    # Test 12: Logout
    print("\n12. Testing logout...")
    response = requests.post(f"{BASE_URL}/auth/logout", headers={"Authorization": f"Bearer {user_token}"})

    if response.status_code == 204:
        print("✓ Logout successful")

        # Test that token no longer works
        response = requests.get(f"{BASE_URL}/users/profile", headers={"Authorization": f"Bearer {user_token}"})
