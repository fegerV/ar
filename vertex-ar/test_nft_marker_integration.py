    ...
    try:
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_out:
                ...
                marker_subdir = os.path.join(
                    nft_markers_dir, os.path.basename(temp_img_path).split(".")[0]
                ...
                if os.path.exists(marker_subdir):
                    ...
                    assert (
                        len(marker_files) >= 3
                    ), f"Должно быть создано как минимум 3 файла маркера, создано: {len(marker_files)}"
                else:
...
def main():
    ...
    tests = [test_nft_marker_generator, test_generate_nft_marker_function, test_nft_marker_config]
