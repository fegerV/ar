...
class TestDatabase:
    ...
    def test_initialise_schema(self, temp_db):
        ...
        cursor = temp_db._execute("SELECT name FROM sqlite_master WHERE type='table'")
        tables = [row[0] for row in cursor.fetchall()]

        expected_tables = ["users", "ar_content", "clients", "portraits", "videos"]
    ...
    def test_video_operations(self, temp_db):
        ...
        # Create video
        video = temp_db.create_video("video1", "portrait1", "/path/to/video.mp4", is_active=False)
        ...
        video2 = temp_db.create_video("video2", "portrait1", "/path/to/video2.mp4", is_active=False)
