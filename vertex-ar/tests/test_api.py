...
def test_app(temp_db):
    ...
    with patch.dict(
        "os.environ", {"RUNNING_TESTS": "1", "RATE_LIMIT_ENABLED": "false", "CORS_ORIGINS": "http://localhost:8000"}
    ):
        app = create_app()
...
def admin_token(client):
    ...
    # Register admin user
    register_response = client.post("/auth/register", json={"username": "admin", "password": "admin_password"})
...
class TestAuthEndpoints:
    ...
    def test_register_user(self, client):
        ...
        response = client.post("/auth/register", json={"username": "testuser", "password": "test_password"})
    ...
    def test_register_duplicate_user(self, client):
        ...
        # Register first user
        client.post("/auth/register", json={"username": "testuser", "password": "test_password"})

        # Try to register same user again
        response = client.post("/auth/register", json={"username": "testuser", "password": "another_password"})
    ...
    def test_login_valid_credentials(self, client):
        ...
        # Login
        response = client.post("/auth/login", json={"username": "testuser", "password": "test_password"})
        assert response.status_code == 200
    ...
    def test_login_invalid_credentials(self, client):
        ...
        response = client.post("/auth/login", json={"username": "nonexistent", "password": "wrong_password"})
...
class TestClientEndpoints:
    ...
    def test_create_client(self, client, auth_headers):
        ...
        response = client.post("/clients/", json={"phone": "+1234567890", "name": "John Doe"}, headers=auth_headers)
        assert response.status_code == 201
    ...
    def test_create_duplicate_client(self, client, auth_headers):
        ...
        # Create first client
        client.post("/clients/", json={"phone": "+1234567890", "name": "John Doe"}, headers=auth_headers)

        # Try to create duplicate
        response = client.post("/clients/", json={"phone": "+1234567890", "name": "Jane Doe"}, headers=auth_headers)
        assert response.status_code == 409
    ...
    def test_get_client(self, client, auth_headers):
        ...
        # Create a client first
        create_response = client.post("/clients/", json={"phone": "+1234567890", "name": "John Doe"}, headers=auth_headers)
    ...
    def test_search_clients(self, client, auth_headers):
        ...
        client.post("/clients/", json={"phone": "+1234567891", "name": "Jane Doe"}, headers=auth_headers)
    ...
    def test_update_client(self, client, auth_headers):
        ...
        # Update client
        response = client.put(f"/clients/{client_id}", json={"name": "John Smith"}, headers=auth_headers)
        assert response.status_code == 200
...
class TestPortraitEndpoints:
    ...
    def test_create_portrait(self, client, auth_headers):
        ...
        client_response = client.post("/clients/", json={"phone": "+1234567890", "name": "John Doe"}, headers=auth_headers)
        client_id = client_response.json()["id"]

        # Mock file upload and NFT generation
        with (
            patch("app.api.portraits.PreviewGenerator") as mock_preview,
            ...
            mock_nft.return_value.generate_marker.return_value = Mock(fset_path="fset", fset3_path="fset3", iset_path="iset")
            mock_qrcode.make.return_value = Mock()
            mock_qrcode.make.return_value.save = Mock()

            # Mock the BytesIO operations
            with (
                patch("app.api.portraits.BytesIO") as mock_bytesio,
...
class TestUnauthenticatedAccess:
    ...
    def test_unauthenticated_client_create(self, client):
        ...
        response = client.post("/clients/", json={"phone": "+1234567890", "name": "John Doe"})
        assert response.status_code == 401
