    ...
    def decorator(func):
        ...
        async def wrapper(request: Request, *args, **kwargs):
            ...
            if not simple_rate_limiter.is_allowed(key, limit_count, period_seconds):
                raise HTTPException(
                    status_code=429, detail="Rate limit exceeded", headers={"Retry-After": str(period_seconds)}
                )
...
async def rate_limit_dependency(request: Request, limit: str = "100/minute") -> None:
    ...
    key = f"{request.client.host}:{request.url.path}"
    if not simple_rate_limiter.is_allowed(key, limit_count, period_seconds):
        raise HTTPException(status_code=429, detail="Rate limit exceeded", headers={"Retry-After": str(period_seconds)})
...
async def register_user(
    request: Request, user: UserCreate, _: None = Depends(create_rate_limit_dependency(AUTH_RATE_LIMIT))
) -> Dict[str, str]:
...
async def login_user(
    request: Request, credentials: UserLogin, _: None = Depends(create_rate_limit_dependency(AUTH_RATE_LIMIT))
) -> TokenResponse:
...
async def generate_nft_marker(
    ...
    if not marker_name or marker_name.strip() == "":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Marker name is required")
    ...
    try:
        ...
        marker = nft_generator.generate_marker(str(image_path), marker_name.strip(), marker_config)
...
async def delete_video(
    ...
    if video["is_active"]:
        ...
        if len(videos) == 1:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Cannot delete the last active video")
...
async def delete_ar_content(content_id: str, username: str = Depends(require_admin)) -> Dict[str, str]:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Ошибка при удалении файлов: {str(e)}")
    ...
    if database.delete_ar_content(content_id):
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Не удалось удалить контент из базы данных"
        )
...
async def batch_generate_nft_markers(
    request: NFTBatchGenerateRequest, username: str = Depends(require_admin)
    ...
    results = nft_generator.generate_markers_batch(images=images, config=config, max_workers=request.max_workers)
    ...
    return NFTBatchGenerateResponse(results=json_results, successful=successful, failed=failed, total_time=elapsed)
...
async def analyze_image_for_nft(
    image_path: str, use_cache: bool = True, username: str = Depends(require_admin)
) -> NFTAnalysisResponse:
    """
    ...
    return NFTAnalysisResponse(**result, cached=cached)
...
async def generate_nft_preview(image_path: str, username: str = Depends(require_admin)) -> NFTPreviewResponse:
    """
    ...
    try:
        ...
        return NFTPreviewResponse(
            preview_path=str(preview_path), analysis=analysis, feature_count=analysis.get("feature_count", 0)
        )
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to generate preview: {str(e)}")
...
async def get_nft_metrics(username: str = Depends(require_admin)) -> NFTMetricsResponse:
    """
...
async def cleanup_nft_markers(request: NFTCleanupRequest, username: str = Depends(require_admin)) -> NFTCleanupResponse:
    """
    ...
    result = nft_generator.cleanup_unused_markers(used_marker_names=list(used_markers), dry_run=request.dry_run)
...
async def list_nft_config_presets(username: str = Depends(require_admin)) -> List[NFTConfigPresetResponse]:
    """
    ...
    return [
        NFTConfigPresetResponse(name=preset["name"], created_at=preset.get("created_at"), preset_path=preset.get("file_path"))
...
async def save_nft_config_preset(
    request: NFTConfigPresetRequest, username: str = Depends(require_admin)
    ...
    try:
        ...
        return NFTConfigPresetResponse(name=request.preset_name, preset_path=str(preset_path), config=request.config)
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to save preset: {str(e)}")


@app.get("/api/nft-markers/config-presets/{preset_name}", response_model=NFTConfigPresetResponse, tags=["nft-markers"])
async def get_nft_config_preset(preset_name: str, username: str = Depends(require_admin)) -> NFTConfigPresetResponse:
    """
    ...
    try:
        ...
        return NFTConfigPresetResponse(name=preset_name, config=config.to_dict())
    except FileNotFoundError:
...
async def delete_nft_config_preset(preset_name: str, username: str = Depends(require_admin)) -> Dict[str, str]:
    """
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to delete preset: {str(e)}")
...
async def get_nft_analytics(username: str = Depends(require_admin)) -> NFTAnalyticsResponse:
    """
    ...
    if not markers_dir.exists():
        return NFTAnalyticsResponse(total_markers=0, quality_distribution={}, avg_file_sizes={}, total_storage_used=0)
...
async def enhance_image_contrast(
    image_path: str, factor: float = 1.5, username: str = Depends(require_admin)
) -> Dict[str, str]:
    """
    ...
    try:
        enhanced_path = nft_generator.enhance_contrast(image_path=image_path, factor=factor)

        return {"original_path": image_path, "enhanced_path": str(enhanced_path), "factor": str(factor)}
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to enhance contrast: {str(e)}")


@app.post("/api/nft-markers/clear-cache", tags=["nft-markers"])
async def clear_nft_cache(clear_all: bool = False, username: str = Depends(require_admin)) -> Dict[str, Any]:
    """
    ...
    return {"message": f"Cleared {cleared} cache entries", "cleared": cleared, "clear_all": clear_all}
