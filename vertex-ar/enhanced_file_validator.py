...
from audit_logging import AuditLogger, OperationLogger, log_operation, audit_action
...
class EnhancedFileValidator:
    ...
    def _validate_video(
        ...
        if mime_type == 'video/mp4':
            if len(content) < 12:
                raise FileValidationError(
                    "Video file too small to be valid MP4",
                    field="video_content",
                    context={"filename": filename}
                )
            ...
            if content[4:8] != b'ftyp':
                raise FileValidationError(
                    "Invalid MP4 file signature",
                    field="video_signature",
                    context={"filename": filename}
                )
...
class SecureFileStorage:
    ...
    def __init__(
        ...
        validator: Optional[EnhancedFileValidator] = None,
    ...
    async def store_file(
        ...
        **context
        ...
        """
        # Validate file
        file_content, metadata = self.validator.validate_upload_file(
            file, user=user, **context
        )
