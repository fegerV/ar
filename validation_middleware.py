...
from audit_logging import AuditLogger, PerformanceLogger, SecurityLogger
...
class ValidationLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if self.log_responses:
                await self._log_response(request, response, request_id, client_ip, user_agent, start_time)
            ...
            # Log HTTP exception
            await self._log_http_exception(request, http_exc, request_id, client_ip, user_agent, start_time)
            ...
            # Log server error
            await self._log_server_error(request, exc, request_id, client_ip, user_agent, start_time)
        ...
        # Log to audit trail
        await self._log_audit_trail(request, response, exception, request_id, client_ip, user_agent, start_time)
    ...
    async def _log_response(
        self, request: Request, response: Response, request_id: str, client_ip: str, user_agent: str, start_time: float
    ) -> None:
    ...
    async def _log_http_exception(
        self, request: Request, exception: HTTPException, request_id: str, client_ip: str, user_agent: str, start_time: float
    ) -> None:
        """Log HTTP exception."""
        duration_ms = int((time.time() - start_time) * 1000)
    ...
    async def _log_server_error(
        self, request: Request, exception: Exception, request_id: str, client_ip: str, user_agent: str, start_time: float
    ) -> None:
        """Log server error."""
        duration_ms = int((time.time() - start_time) * 1000)
    ...
    def _create_error_response(self, exception: HTTPException) -> JSONResponse:
        ...
        return JSONResponse(
            status_code=exception.status_code,
            content={"detail": exception.detail, "error_code": getattr(exception, "error_code", None)},
        )
...
class InputValidationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if any(request.url.path.startswith(path) for path in self.blocked_paths):
            logger.warning("Blocked path access attempt", path=request.url.path, client_ip=self._get_client_ip(request))
    ...
    async def _validate_and_sanitize_request(self, request: Request) -> None:
        ...
        if "application/json" in content_type and self.validate_json:
            try:
                ...
                logger.warning(
                    "Invalid JSON in request", path=request.url.path, error=str(e), client_ip=self._get_client_ip(request)
                )
            ...
            if isinstance(value, str):
                for pattern in injection_patterns:
                    if re.search(pattern, value, re.IGNORECASE):
                        ...
                        raise HTTPException(
                            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid content detected in field: {key}"
                        )
                ...
                if self.sanitize_input:
                    try:
                        ...
                        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid input in field: {key}")
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if "X-RateLimit-Limit" in response.headers:
            ...
            if remaining < limit * 0.1:  # Less than 10% remaining
                logger.warning("Rate limit nearly exceeded", **log_data)
