...
class TestNFTGeneration(unittest.TestCase):
    ...
    def create_sample_image(self, path):
        ...
        cv2.rectangle(image, (50, 50), (250, 250), (0, 255, 0), 3)  # Green rectangle
        cv2.circle(image, (150, 150), 75, (0, 0, 255), 3)  # Red circle
    ...
    def test_generate_nft_marker_output_files_content(self):
        ...
        # Check content of .iset file
        marker_name = Path(self.sample_image_path).stem
        iset_path = os.path.join(self.output_dir, f"{marker_name}.iset")
        self.assertTrue(os.path.exists(iset_path))
        
        with open(iset_path, 'r') as f:
            iset_content = f.read()
            
        # Check that .iset file contains expected information
        self.assertIn("# ImageSet file", iset_content)
        self.assertIn("width:", iset_content)
        self.assertIn("height:", iset_content)
        ...
        with open(fset_path, 'r') as f:
            fset_content = f.read()
            
        # Check that .fset file contains expected information
        self.assertIn("# FeatureSet file", fset_content)
        ...
        with open(fset3_path, 'r') as f:
            fset3_content = f.read()
            
        # Check that .fset3 file contains expected information
        self.assertIn("# FeatureSet3 file", fset3_content)
