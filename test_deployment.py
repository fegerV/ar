...
def test_local_application_launch():
    ...
    try:
        ...
        process = subprocess.Popen([sys.executable, "-m", "main"], cwd=dst_dir, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def test_docker_launch():
    ...
    try:
        ...
        build_process = subprocess.run(
            ["docker", "build", "-t", "vertex-ar:test", "."], cwd="vertex-ar", capture_output=True, text=True
        )
        ...
        if build_process.returncode == 0:
            ...
            run_process = subprocess.run(
                ["docker", "run", "-d", "-p", "8000:8000", "--name", "vertex-ar-test", "vertex-ar:test"],
                capture_output=True,
                text=True,
            )
            ...
            if run_process.returncode == 0:
                ...
                subprocess.run(["docker", "stop", "vertex-ar-test"], capture_output=True)
                subprocess.run(["docker", "rm", "vertex-ar-test"], capture_output=True)
...
def test_application_after_restart():
    ...
    try:
        ...
        process1 = subprocess.Popen(
            [sys.executable, "-m", "main"], cwd=dst_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
...
def test_application_logs():
    ...
    try:
        ...
        with open(log_file, "w") as log_f:
            process = subprocess.Popen([sys.executable, "-m", "main"], cwd=dst_dir, stdout=log_f, stderr=subprocess.STDOUT)
...
def main():
    ...
    tests = [test_local_application_launch, test_docker_launch, test_application_after_restart, test_application_logs]
