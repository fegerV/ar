...
class StorageIntegrationTester:
    ...
    def test_local_storage_upload(self, file_set: Dict) -> StorageTestResult:
        ...
        try:
            with open(file_set["image"], "rb") as img, open(file_set["video"], "rb") as vid:
                ...
                data = {"phone": file_set["phone"], "name": file_set["name"]}
    ...
    def test_minio_direct_upload(self, file_set: Dict) -> StorageTestResult:
        ...
        try:
            ...
            self.minio_client.fput_object(bucket_name, image_object, file_set["image"])

            # Загружаем видео
            video_object = f"test_videos/{timestamp}_video.mp4"
            self.minio_client.fput_object(bucket_name, video_object, file_set["video"])
    ...
    def test_minio_download(self, file_set: Dict) -> StorageTestResult:
        ...
        try:
            ...
            # Скачиваем файл
            self.minio_client.fget_object(bucket_name, video_object, download_path)
    ...
    def test_storage_performance_comparison(self) -> bool:
        ...
        if self.minio_client:
            ...
            if minio_upload_result.success:
                print(
                    f"    ✅ Загрузка: {minio_upload_result.duration:.2f} сек ({minio_upload_result.upload_speed_mbps:.2f} МБ/с)"
                )
            else:
            ...
            if minio_download_result.success:
                print(
                    f"    ✅ Скачивание: {minio_download_result.duration:.2f} сек ({minio_download_result.download_speed_mbps:.2f} МБ/с)"
                )
            else:
