...
def test_ar_content_listing():
    ...
    try:
        ...
        if len(user1_content) == 1 and len(user2_content) == 1:
            ...
            print(f"  Пользователи должны видеть по 1 элементу, но user1 видит {len(user1_content)}, user2 видит {len(user2_content)}: ОШИБКА")
            return False
...
def test_storage_info():
    ...
    try:
        ...
        from utils import get_storage_usage, format_bytes
