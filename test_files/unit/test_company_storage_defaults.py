...
class TestCompanyCreateDefaults:
    ...
    def test_accepts_minio(self):
        """Test CompanyCreate accepts 'minio' as storage_type with connection_id."""
        company = CompanyCreate(name="Test Company", storage_type="minio", storage_connection_id="test-conn-123")
        assert company.storage_type == "minio"

    def test_accepts_yandex_disk(self):
        """Test CompanyCreate accepts 'yandex_disk' as storage_type with connection_id and folder_id."""
        company = CompanyCreate(
            name="Test Company",
            storage_type="yandex_disk",
            storage_connection_id="test-conn-123",
...
class TestDatabaseDefaults:
    ...
    def test_create_company_explicit_local_legacy(self, temp_db):
        ...
        # Note: The database normalizes 'local' to 'local_disk' automatically
        assert company["storage_type"] == "local_disk"
...
class TestDefaultCompanyImmutability:
    """Test that default company storage settings cannot be changed."""
    ...
    def test_default_company_storage_type_immutable(self, temp_db):
        """Test that default company storage type cannot be changed to non-local_disk."""
        # Try to update default company storage type to minio
        success = temp_db.update_company(
            "vertex-ar-default",
            storage_type="minio"
        )
        # Update should succeed but not change the storage type
        company = temp_db.get_company("vertex-ar-default")
        assert company["storage_type"] == "local_disk"

        # Try to update to yandex_disk
        success = temp_db.update_company(
            "vertex-ar-default",
            storage_type="yandex_disk"
        )
    ...
    def test_default_company_storage_folder_path_immutable(self, temp_db):
        """Test that default company storage folder path cannot be changed."""
        # Try to update default company storage folder path
        success = temp_db.update_company(
            "vertex-ar-default",
            storage_folder_path="custom_folder"
        )
        # Update should succeed but not change the storage folder path
        company = temp_db.get_company("vertex-ar-default")
        assert company["storage_folder_path"] == "vertex_ar_content"

    def test_default_company_can_update_other_fields(self, temp_db):
        """Test that default company can still update other fields."""
        # Update other fields should work
        success = temp_db.update_company(
            "vertex-ar-default",
            name="Vertex AR Updated",
        ...
        company = temp_db.get_company("vertex-ar-default")
        assert company["name"] == "Vertex AR Updated"
        assert company["email"] == "new@example.com"
        assert company["description"] == "Updated description"
        # Storage settings should remain unchanged
        assert company["storage_type"] == "local_disk"
        assert company["storage_folder_path"] == "vertex_ar_content"

    def test_default_company_corrected_on_ensure(self, temp_db):
        """Test that ensure_default_company corrects storage settings if changed."""
        # Manually change the storage settings
        temp_db._connection.execute(
            "UPDATE companies SET storage_type = ?, storage_folder_path = ? WHERE id = ?",
            ("minio", "wrong_folder", "vertex-ar-default")
        )
        temp_db._connection.commit()
        ...
        company = temp_db.get_company("vertex-ar-default")
        assert company["storage_type"] == "minio"
        assert company["storage_folder_path"] == "wrong_folder"

        # Run ensure_default_company which should correct the settings
        from app.database import ensure_default_company
        ensure_default_company(temp_db)

        # Verify settings were corrected
        company = temp_db.get_company("vertex-ar-default")
        assert company["storage_type"] == "local_disk"
        assert company["storage_folder_path"] == "vertex_ar_content"
