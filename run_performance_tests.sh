#!/bin/bash

# Скрипт для запуска всех тестов производительности
# Включает комплексное тестирование, нагрузочное тестирование и тестирование хранилищ

set -e

echo "🚀 ЗАПУСК КОМПЛЕКСНОГО ТЕСТИРОВАНИЯ ПРОИЗВОДИТЕЛЬНОСТИ"
echo "=================================================="

# Проверяем наличие необходимых зависимостей
echo "📦 Проверка зависимостей..."
python3 -c "import psutil, requests, PIL" 2>/dev/null || {
    echo "❌ Отсутствуют необходимые зависимости"
    echo "Установите их командой:"
    echo "pip install psutil requests pillow minio memory-profiler"
    exit 1
}

echo "✅ Все зависимости установлены"

# Проверяем запущено ли приложение
echo "🔍 Проверка запущенного приложения..."
if curl -s http://localhost:8000/health > /dev/null 2>&1; then
    echo "✅ Приложение запущено на http://localhost:8000"
else
    echo "❌ Приложение не запущено на http://localhost:8000"
    echo "Пожалуйста, запустите приложение перед тестированием:"
    echo "cd vertex-ar && python main.py"
    exit 1
fi

# Создаем директорию для отчетов
REPORT_DIR="performance_reports"
mkdir -p "$REPORT_DIR"

echo ""
echo "📊 ТЕСТ 1: Комплексное тестирование производительности с psutil"
echo "=============================================================="
if python3 test_comprehensive_performance.py; then
    mv performance_report.json "$REPORT_DIR/comprehensive_$(date +%Y%m%d_%H%M%S).json"
    echo "✅ Комплексное тестирование завершено"
else
    echo "❌ Комплексное тестирование не пройдено"
fi

echo ""
echo "🔄 ТЕСТ 2: Нагрузочное тестирование API портретов"
echo "=================================================="
if python3 test_portraits_load.py; then
    mv load_test_report.json "$REPORT_DIR/load_test_$(date +%Y%m%d_%H%M%S).json"
    echo "✅ Нагрузочное тестирование завершено"
else
    echo "❌ Нагрузочное тестирование не пройдено"
fi

echo ""
echo "🗄️  ТЕСТ 3: Тестирование интеграции с MinIO/локальным хранилищем"
echo "================================================================"
if python3 test_storage_integration.py; then
    mv storage_integration_report.json "$REPORT_DIR/storage_$(date +%Y%m%d_%H%M%S).json"
    echo "✅ Тестирование хранилищ завершено"
else
    echo "❌ Тестирование хранилищ не пройдено"
fi

echo ""
echo "📈 ТЕСТ 4: Базовое тестирование производительности"
echo "=================================================="
if python3 test_performance.py; then
    echo "✅ Базовое тестирование завершено"
else
    echo "❌ Базовое тестирование не пройдено"
fi

echo ""
echo "🎉 ВСЕ ТЕСТЫ ПРОИЗВОДИТЕЛЬНОСТИ ЗАВЕРШЕНЫ"
echo "=========================================="
echo "Отчеты сохранены в директории: $REPORT_DIR"
echo ""
echo "📊 Анализ результатов:"
echo "  - Комплексное тестирование: CPU, память, дисковый I/O"
echo "  - Нагрузочное тестирование: конкурентные запросы, стресс-тест"
echo "  - Тестирование хранилищ: MinIO vs локальное хранилище"
echo "  - Базовое тестирование: большие файлы, множественные операции"
echo ""
echo "💡 Рекомендации:"
echo "  - Проверьте отчеты в $REPORT_DIR для детального анализа"
echo "  - Обратите внимание на утечки памяти и пиковую нагрузку CPU"
echo "  - Сравните производительность MinIO и локального хранилища"
echo "  - Проанализируйте результаты нагрузочного тестирования"
